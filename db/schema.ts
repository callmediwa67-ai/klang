...
export const categories = sqliteTable("categories", { id: text("id").primaryKey(), name: text("name").notNull(), normalizedName: text("normalized_name").notNull(), color: text("color").notNull().default("neutral"), createdByName: text("created_by_name").notNull(), createdByTeam: text("created_by_team").notNull(), createdAt: text("created_at").notNull(), archivedAt: text("archived_at") }, (table) => [uniqueIndex("idx_categories_normalized_name_unique").on(table.normalizedName), index("idx_categories_active_name").on(table.archivedAt, table.name)]);
