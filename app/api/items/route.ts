...
export async function PATCH(request: Request) {
  try { const payload = await request.json() as Record<string, unknown>; const actor = parseActor(payload.actor); const id = cleanText(payload.id, 100); const expectedVersion = Number(payload.expectedVersion);
    ...
    else { if (current.deletedAt) return error("รายการนี้อยู่ในถังขยะแล้ว", 409); if (typeof payload.title === "string") { const title = cleanText(payload.title, 160); if (!title) return error("ชื่อรายการห้ามว่าง"); changes.title = title; } if (typeof payload.content === "string") changes.content = cleanText(payload.content, 12_000); if (typeof payload.url === "string") { const url = validUrl(cleanText(payload.url, 2_000)); if (payload.url && !url) return error("ลิงก์ไม่ถูกต้อง"); changes.url = url; } if (typeof payload.category === "string") { const category = cleanText(payload.category, 40); if (!category) return error("หมวดหมู่ห้ามว่าง"); changes.category = category; } if (typeof payload.inbox === "boolean") changes.inbox = payload.inbox; if (typeof payload.favorite === "boolean") changes.favorite = payload.favorite; }
