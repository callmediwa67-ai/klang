...
import { itemLinks, items, itemVersions, type VaultItem } from "../../../db/schema";
...
function parseUrls(value: unknown) { const values = Array.isArray(value) ? value : typeof value === "string" ? [value] : []; return Array.from(new Set(values.map((entry) => validUrl(cleanText(entry, 2_000))).filter(Boolean))).slice(0, 50); }
...
async function getLinks(itemId: string) { return getDb().select().from(itemLinks).where(eq(itemLinks.itemId, itemId)).orderBy(itemLinks.sortOrder); }
async function replaceLinks(itemId: string, urls: string[]) { const db = getRawDb(); await db.prepare("DELETE FROM item_links WHERE item_id = ?").bind(itemId).run(); if (urls.length) await db.batch(urls.map((url, sortOrder) => db.prepare("INSERT INTO item_links (id, item_id, url, sort_order, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), itemId, url, sortOrder, new Date().toISOString()))); }
...
export async function GET(request: Request) {
  try { await ensureDatabase(); const query = new URL(request.url).searchParams; const id = cleanText(query.get("id"), 100);
    if (query.get("history") === "1" && id) { const [item] = await getDb().select().from(items).where(eq(items.id, id)); if (!item) return error("ไม่พบรายการนี้", 404); const [versions, links] = await Promise.all([getDb().select().from(itemVersions).where(eq(itemVersions.itemId, id)).orderBy(desc(itemVersions.versionNumber)), getLinks(id)]); return Response.json({ item, versions, links }); }
    const view = cleanText(query.get("view"), 20); const search = cleanText(query.get("q"), 160); const category = cleanText(query.get("category"), 40); const page = Math.max(1, Math.min(Number(query.get("page")) || 1, 10_000)); const pageSize = Math.max(1, Math.min(Number(query.get("pageSize")) || 30, 100));
    const trash = view === "trash";
    const filters = [trash ? "deleted_at IS NOT NULL" : "deleted_at IS NULL"]; const bindings: unknown[] = [];
    if (!trash && view === "inbox") filters.push("inbox = 1");
    if (!trash && view === "favorites") filters.push("favorite = 1");
    if (category && category !== "all") { filters.push("category = ?"); bindings.push(category); }
...
export async function POST(request: Request) {
  try { const payload = await request.json() as Record<string, unknown>; const actor = parseActor(payload.actor); if (!actor) return error("กรุณาระบุชื่อและทีมก่อนบันทึก"); await ensureDatabase(); if (!await enforceWriteRateLimit(request, actor)) return error("ทำรายการเร็วเกินไป กรุณารอสักครู่", 429);
    const type = cleanText(payload.type, 20); const title = cleanText(payload.title, 160); const content = cleanText(payload.content, 12_000); const category = cleanText(payload.category, 40) || "uncategorized"; const urls = parseUrls(payload.urls ?? payload.url); const url = urls[0] ?? "";
    if (!itemTypes.has(type) || !title) return error("กรุณาระบุประเภทรายการและชื่อ"); if (type === "link" && !url) return error("กรุณาใส่ลิงก์ http หรือ https ที่ถูกต้อง");
    const now = new Date().toISOString(); const [item] = await getDb().insert(items).values({ id: crypto.randomUUID(), type: type as VaultItem["type"], title, content, url, category, inbox: true, favorite: false, version: 1, createdAt: now, updatedAt: now, createdByDeviceId: actor.deviceId, createdByName: actor.name, createdByTeam: actor.team, updatedByDeviceId: actor.deviceId, updatedByName: actor.name, updatedByTeam: actor.team }).returning(); if (type === "link") await replaceLinks(item.id, urls); await recordVersion(item, actor); await recordActivity(actor, "created", item); return Response.json({ item, links: type === "link" ? await getLinks(item.id) : [] }, { status: 201 });
  } catch (reason) { return serverError(reason); }
}
...
export async function PATCH(request: Request) {
  try { const payload = await request.json() as Record<string, unknown>; const actor = parseActor(payload.actor); const id = cleanText(payload.id, 100); const expectedVersion = Number(payload.expectedVersion);
    ...
    else { if (current.deletedAt) return error("รายการนี้อยู่ในถังขยะแล้ว", 409); if (typeof payload.title === "string") { const title = cleanText(payload.title, 160); if (!title) return error("ชื่อรายการห้ามว่าง"); changes.title = title; } if (typeof payload.content === "string") changes.content = cleanText(payload.content, 12_000); if (Array.isArray(payload.urls)) { const urls = parseUrls(payload.urls); if (current.type === "link" && !urls.length) return error("กรุณาใส่ลิงก์ http หรือ https ที่ถูกต้อง"); changes.url = urls[0] ?? ""; } else if (typeof payload.url === "string") { const url = validUrl(cleanText(payload.url, 2_000)); if (payload.url && !url) return error("ลิงก์ไม่ถูกต้อง"); changes.url = url; } if (typeof payload.category === "string") { const category = cleanText(payload.category, 40); if (!category) return error("หมวดหมู่ห้ามว่าง"); changes.category = category; } if (typeof payload.inbox === "boolean") changes.inbox = payload.inbox; if (typeof payload.favorite === "boolean") changes.favorite = payload.favorite; }
    const [item] = await getDb().update(items).set(changes).where(and(eq(items.id, id), eq(items.version, expectedVersion))).returning(); if (!item) { const [latest] = await getDb().select().from(items).where(eq(items.id, id)); return Response.json({ error: "มีคนแก้ไขรายการนี้ก่อนคุณ", current: latest }, { status: 409 }); } if (Array.isArray(payload.urls) && item.type === "link") await replaceLinks(item.id, parseUrls(payload.urls)); await recordVersion(item, actor); await recordActivity(actor, action, item); return Response.json({ item, links: item.type === "link" ? await getLinks(item.id) : [] });
  } catch (reason) { return serverError(reason); }
}
